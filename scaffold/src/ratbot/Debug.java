 ...
 *
 * <p>Features: - Visual indicators in client (dots, lines, strings) - Debug levels (VERBOSE, INFO,
 * WARNING, ERROR) - Conditional compilation (disable in production) - Zero overhead when disabled
 *
 * <p>Usage: Debug.setLevel(Debug.Level.VERBOSE); Debug.info(rc, "State transition: EXPLORE ->
 * COLLECT"); Debug.dot(rc, location, Debug.Color.GREEN); Debug.line(rc, start, end,
 * Debug.Color.RED);
 */
public class Debug {
  ...
  public static void debugPath(
      RobotController rc, MapLocation start, MapLocation target, Direction step) {
    if (!DEBUG_ENABLED) return;
  ...
  public static void debugStateChange(
      RobotController rc, String fromState, String toState, String reason) {
    if (!DEBUG_ENABLED) return;
