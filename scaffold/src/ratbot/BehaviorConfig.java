 ...
 *
 * <p>Centralizes magic numbers with documentation and rationale. Enables easy tuning based on match
 * data and strategy evolution.
 *
 * <p>All values derived from analysis and testing (see DECISIONS.md).
 */
public class BehaviorConfig {
   ...
   *
   * <p>Rationale: Balance carrying penalty vs trip overhead - Carrying penalty: 0.01 * cheese (at
   * 20 = 1.2x slower movement) - Round-trip overhead: ~30 rounds to mine and back - Optimization
   * space: 15-25 range based on map size
   *
   * <p>Tuning: Revisit after Sprint 1 with match data - If cheese income high: Increase to 30
   * (fewer trips) - If maps small: Decrease to 15 (less penalty)
   */
   ...
   *
   * <p>Rationale: Faster deliveries prevent starvation - In emergency: Minimize carrying time -
   ...
   *
   * <p>Rationale: More frequent deliveries without extreme overhead - Balances urgency with
   * efficiency
   */
   ...
   *
   * <p>Rationale: GameTheory.evaluate() costs ~100 bytecode - Checking every 50 rounds = 2
   * bytecode/turn average overhead - Strategic decisions don't change rapidly (50-round granularity
   * acceptable)
   *
   * <p>Tuning: Can reduce to 25 rounds if more responsive backstab needed
   */
   ...
   *
   * <p>Rationale: Early game backstab is suboptimal - Before round 200: Cats still strong, cat
   * damage critical - Cooperation mode values cat damage at 50% vs 30% in backstab - Early backstab
   * sacrifices 20% scoring weight
   *
   * <p>Tuning: May adjust based on cat defeat rate - If cats weak early: Reduce to 100 - If cats
   * strong: Increase to 300
   */
   ...
   *
   * <p>Rationale: Risk mitigation for strategic shift - Require clear advantage before committing -
   * Account for estimation errors in enemy state
   *
   * <p>Tuning: Conservative (10 points) for Sprint 1 - Can reduce to 5 if game theory accurate -
   ...
   *
   * <p>Rationale: Circuit breaker trigger point - 33 rounds = minimum survival buffer - Allows time
   * for emergency response - Below this: CRITICAL mode (stop all spawning)
   */
   ...
   *
   * <p>Rationale: Early warning for cheese shortages - 100 rounds = comfortable buffer - Triggers
   * reduced spawning, increased collection priority - Prevents critical emergencies
   */
   ...
   *
   * <p>Rationale: Safety margin beyond rounds calculation - Even with good rounds remaining, need
   * absolute minimum - Accounts for sudden cheese consumption (upgrades, attacks)
   */
   ...
   *
   * <p>Rationale: New king sustainability - Formation cost: 50 cheese - Buffer for 500 rounds:
   * 1,500 cheese - Total requirement: 1,550 cheese per new king
   */
   ...
   *
   * <p>Rationale: Balance visibility with log volume - Every 20 rounds captures key transitions -
   * Reduces log spam in 2,000-round games - Profiler samples at same rate for correlation
   */
   ...
   *
   * <p>Rationale: Track cheese income trends - Every 10 rounds enables income rate calculation -
   ...
   *
   * <p>Rationale: Prevent multi-king cat pounce losses - Cat pounce range: sqrt(9) = 3 tiles - Safe
   * spacing: 15 tiles (5x pounce range) - Provides spatial redundancy
   *
   * <p>Tuning: Based on cat behavior analysis - If cats rarely target kings: Reduce to 10 - If
   * multi-king losses observed: Increase to 20
   */
   ...
   *
   * <p>Rationale: Balance spawn rate with consumption - Small maps (<30x30): 1-2 kings sufficient -
   * Medium maps (30x40): 2-3 kings optimal - Large maps (40x60): 3 kings maximum (consumption
   * limit)
   */
   ...
   *
   * <p>Rationale: Preserve kings at all costs - Below 40% health: High death risk - Kings worth 50%
   * of score in backstab mode - Losing king = instant loss if last one
   */
   ...
   *
   * <p>Rationale: Only enhance when guarantees kill - Base damage: 10 - Enhancement:
   * ceil(log2(cheese)) bonus - Only spend if: damage >= target.health
   *
   * <p>Use: GameTheory.worthEnhancingBite()
   */
   ...
   *
   * <p>Rationale: Reduce profiling overhead - Full profiling: ~50 bytecode overhead per section -
   ...
   *
   * <p>Rationale: Early warning for bytecode pressure - Baby rat limit: 17,500 - Warning at 80% =
   * 14,000 bytecode - Allows fallback to cheaper algorithms
   */
   ...
   *
   * <p>Array size: 64 slots × 10 bits (0-1023 per slot)
   */
