 ...
 *
 * Core mechanic: Cooperation vs Backstabbing
 * - Cooperation scoring: 50% cat damage + 30% kings + 20% cheese
 * - Backstabbing scoring: 30% cat damage + 50% kings + 20% cheese
 *
 * Optimal backstab timing is a key strategic decision.
 * Standalone module for pre-scaffold analysis.
 */
public class GameTheory {
    ...
    /**
     * Calculate backstabbing mode score.
     * Assumes we eventually win the king battle (optimistic).
     *
     * @param ourCatDamage Damage we did to cats
     * @param enemyCatDamage Damage enemy did to cats
     * @param ourKings Number of our living rat kings
    ...
    public static boolean shouldBackstab(
        ...
        int coopScore = scoreCooperation(
            ourCatDamage, enemyCatDamage,
            ourKings, enemyKings,
            ourCheese, enemyCheese
        );

        int backstabScore = scoreBackstabbing(
            ourCatDamage, enemyCatDamage,
            ourKings, enemyKings,
            ourCheese, enemyCheese
        );
    ...
    public static BackstabRecommendation evaluate(GameState state) {
        ...
        if (delta > 20) {
            ...
            return new BackstabRecommendation(true, delta, "King advantage (" + state.ourKings + " vs " + state.enemyKings + ")");
        } else {
            return new BackstabRecommendation(false, delta, "Not enough advantage (" + delta + " points)");
        }
    }

    /**
     * Simulate optimal backstab timing over game.
     * Returns round with maximum expected score delta.
     *
    ...
    public static class GameState {
        ...
        public GameState(int round, int ourCatDmg, int enemyCatDmg,
                        int ourKings, int enemyKings,
                        int ourCheese, int enemyCheese) {
            this.round = round;
    ...
    /**
     * Estimate cheese value per damage to cat.
     * Helps decide if spending cheese on bite enhancement is worth it.
     *
