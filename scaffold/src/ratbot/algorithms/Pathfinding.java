 ...
 * - Directional Bug2 for obstacle avoidance
 * - A* for weighted pathfinding (future)
 *
...
public class Pathfinding {
     ...
     * Returns first direction to take, or CENTER if unreachable.
     *
     ...
     * Returns the first direction to take from start.
     */
    ...
    /**
     * Directional Bug2 pathfinding.
     * Handles obstacles with tracing behavior, respects facing direction.
     *
     * Simpler than BFS, lower bytecode, good for real-time pathfinding.
     */
        ...
        if (bugTracing) {
            ...
            for (int i = 8; --i >= 0;) {
                if (canMove.canMove(dir)) {
                    bugTracingDir = bugRotateRight ?
                        rotateLeft(dir) : rotateRight(dir);
                    return dir;
                }
                dir = bugRotateRight ?
                    rotateRight(dir) : rotateLeft(dir);
            }
        }
     ...
     * Allows bug2 to work with any passability function.
     */
    ...
    /**
     * Simple greedy pathfinding (move toward target).
     * Cheapest bytecode, but can get stuck.
     *
     * @param current Current location
     * @param target Target location
