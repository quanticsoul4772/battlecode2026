 ...
 *
 * <p>Optimized for bytecode efficiency: - Backward loops (30% savings) - Avoid allocations in hot
 * paths - Integer math (no floating point)
 *
 * <p>Standalone module - integrates into any scaffold.
 */
public class Geometry {
  ...
  /**
   * Manhattan distance (taxicab distance). Cheaper than Euclidean, useful for pathfinding
   * heuristics.
   *
  ...
  /**
   * Chebyshev distance (chessboard distance). max(|dx|, |dy|) - useful for square vision ranges.
   *
   * @param a First location
   * @param b Second location
   * @return max(|dx|, |dy|)
  ...
  /**
   * Euclidean distance squared. Standard distance metric, avoids sqrt (expensive).
   *
   * @param a First location
   * @param b Second location
   * @return dx² + dy²
  ...
  /**
   * Find closest location to reference point. Uses backward loop for bytecode efficiency.
   *
  ...
  /**
   * Sort locations by distance from reference (in-place). Uses simple insertion sort (good for
   * small arrays).
   *
   * @param reference Reference point
  ...
  /**
   * Check if location is within square radius. Faster than distanceSquaredTo for range checks.
   *
  ...
  public static int locationsWithinRadiusIntoBuffer(
      MapLocation[] buffer, MapLocation center, int radiusSquared, int mapWidth, int mapHeight) {
    // Bounding box
   ...
   * better performance, use locationsWithinRadiusIntoBuffer() with your own buffer.
   *
   * @param center Center point
   * @param radiusSquared Radius squared
   * @param mapWidth Map width
  ...
  public static int locationsWithinRadiusCount(
      MapLocation center, int radiusSquared, int mapWidth, int mapHeight) {
    return locationsWithinRadiusIntoBuffer(
        radiusBuffer, center, radiusSquared, mapWidth, mapHeight);
  }

  /**
   * Access location at index from static buffer. Only valid after calling
   * locationsWithinRadiusCount().
   *
  ...
  /**
   * Get all locations within radius (DEPRECATED - allocates memory). Use
   * locationsWithinRadiusCount() + getLocationInRadius() for better performance.
   *
   * @param center Center point
   * @param radiusSquared Radius squared
   * @param mapWidth Map width
  ...
  /**
   * Fast approximate distance (avoids sqrt). Octagonal approximation: max + 0.414*min Accurate
   * within ~8%.
   *
   * @param dx Delta X
   * @param dy Delta Y
  ...
  /**
   * Check if path from A to B is clear (no obstacles). Simple ray-casting for line-of-sight.
   *
   ...
   */
  public static boolean isLineOfSightClear(MapLocation from, MapLocation to, boolean[][] passable) {
