 ...
 *
 * <p>Keep it simple: spawn, collect, deliver, survive. No zones, no queues, no fancy coordination.
 */
    ...
    if (lastX != 0) {
      ...
      if (dist <= 50) {
        ...
        MapLocation searchTarget =
            sharedPos.add(searchDirection).add(searchDirection).add(searchDirection);
    ...
    if (rc.canMove(desired)) {
      ...
      if (facing == desired) {
        ...
        if (rc.canSenseLocation(nextLoc)) {
          ...
          else if (blocker != null && blocker.getTeam() == rc.getTeam().opponent())
            blockType = "ENEMY_RAT";
