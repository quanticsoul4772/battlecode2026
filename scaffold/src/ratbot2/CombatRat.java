 ...
 *
 * <p>NEW OBJECTIVE: DISTRACT cats (keep them away from king). Strategy: Patrol between cat and
 * king, attract cat attention, kite cat away.
 */
public class CombatRat {
      ...
      for (RobotInfo enemy : visibleEnemies) {
        if (enemy.getType() == UnitType.RAT_KING) {
          ...
          if (rc.canAttack(enemyLoc)
              && Vision.canSee(me, rc.getDirection(), enemyLoc, UnitType.BABY_RAT)) {
            rc.attack(enemyLoc);
            System.out.println(
                "KING_ASSAULT:" + rc.getRoundNum() + ":" + rc.getID() + ":attacking enemy king!");
            return;
          }
        }
      }
  ...
  private static void defendKing(RobotController rc, RobotInfo[] enemies)
    ...
    for (RobotInfo enemy : enemies) {
      if (enemy.getLocation().distanceSquaredTo(kingLoc)
          < nearest.getLocation().distanceSquaredTo(kingLoc)) {
        nearest = enemy;
      }
    }
    ...
    // Attack if adjacent
    if (me.distanceSquaredTo(enemyLoc) <= 2
        && Vision.canSee(me, rc.getDirection(), enemyLoc, UnitType.BABY_RAT)) {
      if (rc.canAttack(enemyLoc)) {
        rc.attack(enemyLoc);
  ...
  private static void distractCats(RobotController rc) throws GameActionException {
    ...
    if (visibleCat != null) {
      ...
      if (rc.getRoundNum() % 20 == 0) {
        System.out.println(
            "DISTRACT:" + rc.getRoundNum() + ":" + rc.getID() + ":patrolling center");
      }
      Movement.moveToward(rc, center);
    }
  }
  ...
  /**
   * Attack primary target cat (focus fire). CRITICAL: Fight cats at MAP CENTER, not near king.
