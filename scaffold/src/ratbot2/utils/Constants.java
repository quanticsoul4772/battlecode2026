...
/**
 * Battlecode 2026 Game Constants. Source: Official javadoc constant-values.html + specs.pdf
 *
 * <p>Use these exact values for calculations.
 */
public class Constants {
  ...
  /**
   * Calculate spawn cost based on alive baby rats. Formula: 10 + 10 * floor(baby_rats_alive / 4)
   */
  public static int getSpawnCost(int babyRatsAlive) {
    return BUILD_ROBOT_BASE_COST
        + BUILD_ROBOT_COST_INCREASE * (babyRatsAlive / NUM_ROBOTS_FOR_COST_INCREASE);
  }

  /**
   * Calculate cheese spawn probability for a mine. Formula: 1 - (1 - 0.01)^r where r = rounds since
   * last spawn
   */
  ...
  /** Calculate bite damage with cheese enhancement. Formula: 10 + ceil(log2(cheese)) */
  ...
  /**
   * Calculate throw damage based on airtime. Formula: 5 * (4 - airtime) Note: This is per the PDF
   * spec, but javadoc shows THROW_DAMAGE = 20 (max)
   */
  ...
  /**
   * Calculate movement cooldown with cheese carrying penalty. Formula: baseCooldown * (1 + 0.01 *
   * cheeseCarried)
   */
  ...
  /** Estimate king sustainability. Returns rounds of cheese supply for given king count. */
