...
/**
 * Movement utilities with advanced pathfinding. Multi-tier: Greedy → Bug2 → BFS (escalating
 * complexity).
 */
public class Movement {
  ...
  /**
   * Move toward target with obstacle avoidance and collision prevention. Returns after turning -
   * move happens next turn (cooldown fix).
   */
      ...
      if (rc.canMove(escape)) {
        if (rc.getDirection() == escape && rc.canMoveForward()) {
          ...
          if (rc.getRoundNum() % 50 == 0) {
            System.out.println(
                "DECROWD:" + rc.getRoundNum() + ":" + rc.getID() + ":escaping crowd");
          }
          return;
        } else if (rc.canTurn()) {
          rc.turn(escape);
        ...
        if (rc.getDirection() == dir) {
          if (rc.canMoveForward()) {
            ...
            if (rc.getRoundNum() % 100 == 0) {
              System.out.println(
                  "MOVE_ALT:" + rc.getRoundNum() + ":" + rc.getID() + ":escaped via " + dir);
            }
            return;
          }
        } else if (rc.canTurn()) {
          ...
          if (rc.getRoundNum() % 100 == 0) {
            System.out.println(
                "MOVE_TURN:" + rc.getRoundNum() + ":" + rc.getID() + ":turning to " + dir);
          }
          return;
        }
      }
  ...
  /** Advanced pathfinding with tiered algorithms. Greedy → Bug2 → BFS (escalates if stuck). */
  public static void moveTowardAdvanced(
      RobotController rc, MapLocation target, boolean[][] passable) throws GameActionException {
    MapLocation me = rc.getLocation();
  ...
  /**
   * Check if location is occupied by a friendly rat. Used for collision avoidance to prevent
   * traffic jams.
   */
  private static boolean isFriendlyOccupied(RobotController rc, MapLocation loc)
