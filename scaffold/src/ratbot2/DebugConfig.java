...
/** Debug configuration flags. Toggle features on/off for different debugging scenarios. */
public class DebugConfig {

  // ===== Global Debug Control =====

  /** Master debug switch. Set to false for competition submission (removes all debug overhead). */
  ...
  /** Enable visual indicators in client. Dots, lines, and strings visible in replay. */
  ...
  /** Logging frequency (rounds between verbose logs). Higher = less spam, lower = more detail. */
  public static final int LOG_INTERVAL = 20;

  /**
   * Visual indicator frequency (rounds between indicator updates). Higher = less visual clutter,
   * lower = more real-time feedback.
   */
  ...
  /** Only debug specific robot IDs (empty = all robots). Useful for tracking specific units. */
  public static final int[] DEBUG_ROBOT_IDS = {};

  /** Only debug after specific round (0 = always). Useful for mid/late game debugging. */
  public static final int DEBUG_AFTER_ROUND = 0;

  /** Only debug before specific round (9999 = always). Useful for early game debugging. */
